...
<div class="container">
    @if($isUserLogged)
        <div class="api-token-form">
            <h2>API Token</h2>
            <form action="{{$regenerateTokenUrl}}" method="post">
                @method('POST')
                @csrf
                <div class="form-group">
                    <label>Token</label>
                    <input class="form-control" type="text" value="{{!empty(request()->cookie('token')) ? request()->cookie('token') : 'Please regenerate cookie'}}" disabled>
                </div>
